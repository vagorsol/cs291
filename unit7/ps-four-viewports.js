...
function render() {
	...
	renderer.setViewport(0, 0.5*canvasWidth, 0.5*canvasWidth, 0.5*canvasWidth); // viewport is x coor, y coor, width, height
	renderer.render(scene, frontCam);

	// side view
	sideCam.position.copy(cameraControls.target);
	sideCam.position.z += 1;
	renderer.setViewport(0.5*canvasWidth, 0, 0.5*canvasWidth, 0.5*canvasWidth);
