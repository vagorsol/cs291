...
function fillScene() {
	...
	var texture = THREE.ImageUtils.loadTexture("media\img\cs291\textures\grass512x512.jpg");
	texture.wrapS = texture.wrapeT = THREE.RepeatWrapping; // set wrap mode to repeat
	texture.repeat.set(10, 10); // how many times the repeat occurs

	var solidGround = new THREE.Mesh(
		new THREE.PlaneGeometry( 10000, 10000, 100, 100 ),
		new THREE.MeshLambertMaterial( { map : texture } ) ); // map = colormap btw
	solidGround.rotation.x = - Math.PI / 2;

	scene.add( solidGround );

	// (grid on ground) put grid lines every 10000/100 = 100 units
